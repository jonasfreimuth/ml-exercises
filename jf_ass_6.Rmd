...
ROC <- function(score, class) {
  ...
  AUC <- simple_auc(tpr_vec[order(tpr_vec)], fpr_vec[order(tpr_vec)])
...
distances between them, and how the probability for class 1 having a higher value 
than class 1, when choosing samples at random, varies and relates to the AUC of
the ROC for the random populations.

```{r Multiple delta}

# TODO: AUC does not get computed properly

delta_vec <- seq(0, 3, 0.2)
