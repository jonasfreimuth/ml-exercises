...
ROC <- function(score, class) {
  ...
  for (i in 1:length(score)) {
    ...
    prc_vec[i] <- conf$tp / (conf$tp + conf$fp)
    ysJ_vec[i] <- youdensJ(conf = conf)
    
  }
  
  AUC_ROC <- simple_auc(sort(tpr_vec), sort(fpr_vec))
  AUC_PRC <- simple_auc(sort(prc_vec), sort(tpr_vec))
  
  opar <- par(mfrow = c(2, 2))
  
  plot(pch = 18, fpr_vec[order(tpr_vec)], tpr_vec[order(tpr_vec)],
       main = paste("AUC =", round(AUC_ROC, 3)),
       xlab = "FPR", ylab = "TPR")
  ...
  plot(pch = 18, tpr_vec[order(tpr_vec)], prc_vec[order(tpr_vec)],
       main = paste("AUC =", round(AUC_PRC, 3)),
